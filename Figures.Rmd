...
get_collective_posterior_sequence <- function(data, prior_kappa) {
  collective_posterior <- NULL
  LRE <- cmdstan_model("non_centered_SMART.Stan")
...
get_collective_posterior_multiEstimate_sequence <- function(data,prior_kappa,prior_mu){
 ...
 collective_posterior <- NULL
LRE <- cmdstan_model("non_centered_SMART_MW.stan")
num_methods <- length(levels(as.factor(data$method)))
...
SMART <- function(data, prior_kappa) {
   ### data: y,se,t, method , X
 ### prior_kappa: matrix t,method 0 or 1 (kappa_{method,t})
collective_posterior <-
  get_collective_posterior_sequence(data, prior_kappa)
...
```{r}
n <- 30
sim <- create_data_cor(n,1)

meta_sim <- sim %>% group_by(lab_no) %>% summarize(es = mean(y),var = 0.01,method=method[1])
```
...
meta_sim <- sim %>%
  filter(lab_no <= i) %>%
  group_by(lab_no) %>%
  summarize(es = mean(y), var = 0.01, .groups = "drop")
